...
import { ArrowLeft, ArrowRight, Shuffle, FileCheck2 } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-paper">
      <main className="max-w-2xl mx-auto px-5 sm:px-6 py-8 sm:py-14">
        ...
        <Link
          href="/sinav/pratik"
          className="sticker-card group flex items-center gap-4 rounded-2xl border-2 bg-surface px-5 py-5 mb-8 hover:bg-gold-wash transition-colors"
        >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {exams.map((no) => (
            <Link
              key={no}
              href={`/sinav/${no}`}
              className="sticker-card group flex items-center gap-3 rounded-2xl border-2 bg-surface px-5 py-4 hover:bg-gold-wash transition-colors"
            >
